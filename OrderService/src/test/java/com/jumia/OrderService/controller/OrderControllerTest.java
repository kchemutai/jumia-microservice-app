package com.jumia.OrderService.controller;
...
import com.jumia.OrderService.OrderServiceConfig;
import com.jumia.OrderService.entity.Order;
...
import com.jumia.OrderService.repository.OrderRepository;
import com.jumia.OrderService.service.OrderService;
...
public class OrderControllerTest {
    ...
    public void test_WhenGetOrder_Success() throws Exception {
        ...
        // Fetch persisted order
        Order order = orderRepository.findById(1L).get();

        // ---- Order assertions ----
        assertEquals(order.getId(), actual.getOrderId());
        assertEquals("PLACED", actual.getOrderStatus());
        assertEquals(order.getAmount(), actual.getAmount());
        assertNotNull(actual.getOrderDate());

        // ---- Product assertions (exclude quantity & price) ----
        assertEquals(1, actual.getProductDetails().getProductId());
        assertEquals("iPhone", actual.getProductDetails().getProductName());

        // ---- Payment assertions ----
        assertEquals(PaymentMode.CASH, actual.getPaymentDetails().getPaymentMode());
        assertEquals("SUCCESS", actual.getPaymentDetails().getPaymentStatus());
        assertNotNull(actual.getPaymentDetails().getPaymentDate());
    }
